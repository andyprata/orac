...
!            Ctrl%Ind%Ch_Is(ii) = ibset(Ctrl%Ind%Ch_Is(ii), MixedBit)
   ...
   if (Ctrl%Approach /= AppCld2L) then
      ...
      ! Testing stratospheric (100 hPa appox. 14 km asl) apriori for AshEyj
      ! Ctrl%XB(IPc)         = switch_cls(c, Default=900.,  AshEyj=100., CldIce=400.)
   ...
   if (Ctrl%Approach /= AppCld2L) then
      ...
      ! Ctrl%X0(IPc)         = switch_cls(c, Default=900.,  AshEyj=100., CldIce=400.)
   ...
   if (Ctrl%Approach /= AppCld2L) then
      ...
      !Ctrl%Sx(IPc)         = switch_cls(c, Default=200)
